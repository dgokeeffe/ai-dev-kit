...
@router.post('/agent/invoke', response_model=InvokeAgentResponse)
async def invoke_agent(request: Request, body: InvokeAgentRequest):
  ...
  logger.info(
    'AUTH_DEBUG agent env=%s user_present=%s token_present=%s workspace_url_set=%s',
    os.getenv('ENV', 'development'),
    1 if user_email else 0,
    1 if user_token else 0,
    1 if (workspace_url and workspace_url.strip()) else 0,
  )

  # In production (Databricks Apps), Foundation Model API requires the user's token.
  # Without X-Forwarded-Access-Token we would get "403 Invalid scope" if the agent
  # used service principal credentials.
  if os.getenv('ENV') == 'production' and not user_token:
    raise HTTPException(
      status_code=403,
      detail=(
        'Databricks credentials required for Claude. '
    ...
    try:
      ...
      async for event in stream_agent_response(
        ...
        if event_type == 'text_delta':
          ...
          if is_error and 'Stream closed' in str(content):
            ...
            content = (
              'MCP Connection Lost: The tool execution was '
              'interrupted because the internal communication '
              'channel broke. This usually happens after a '
              'long-running operation. Please start a new '
              'conversation to reset the connection. '
              f'Original error: {content}'
            )
          ...
          # Keepalive during long tool execution
      ...
      if 'Stream closed' in error_message:
        error_message = (
          ...
          'This typically occurs when the Claude subprocess '
          'terminates unexpectedly. Check backend logs for details.'
        )
...
@router.post('/agent/stream_progress/{execution_id}')
async def stream_progress(execution_id: str, body: StreamProgressRequest):
  ...
  if not stream:
    raise HTTPException(status_code=404, detail=f'Stream not found: {execution_id}')
...
@router.post('/agent/stop_stream/{execution_id}', response_model=StopStreamResponse)
async def stop_stream(execution_id: str):
  ...
  if stream.is_complete:
    return StopStreamResponse(success=False, message='Stream already complete')

  cancelled = stream.cancel()

  return StopStreamResponse(
    success=cancelled, message='Stream cancelled' if cancelled else 'Failed to cancel stream'
  )
...
async def get_conversation_executions(
  ...
  return {
    'active': (in_memory_active or (active.to_dict() if active else None)),
    'recent': [e.to_dict() for e in recent if e.id != (active.id if active else None)],
  }
