...
def _get_workspace_client():
    ...
    In Databricks Apps, $HOME may not be set, which causes the SDK's
    databricks-cli auth provider to fail when looking for .databrickscfg.
    We set HOME to /tmp as a workaround so the SDK skips that provider
    and uses the app's service principal credentials.
    """
    try:
        from databricks.sdk import WorkspaceClient

        # Ensure HOME is set so the SDK can look for (and skip) .databrickscfg
...
def ensure_table_permissions() -> None:
    """Ensure the SP has DATABRICKS_SUPERUSER membership on the Lakebase instance.

    Raw Postgres GRANT statements get revoked by Lakebase's ACL reconciliation.
    Instead, we use the Lakebase API to set the SP's membership_role to
    DATABRICKS_SUPERUSER, which persists and gives full table access.
    """
    if not is_dynamic_token_mode():
        return
    ...
    try:
        # Get the SP's identity (username used by the SDK)
        me = client.current_user.me()
        sp_id = me.user_name
        if not sp_id:
        ...
        logger.warning(
            f'Could not ensure DATABRICKS_SUPERUSER membership: {e}. '
            'If you see "permission denied" errors, run:\n'
            f'  ./scripts/grant_lakebase_permissions.sh <app-name> {instance_name}'
        )
