...
def _load_claude_settings() -> dict:
  """Load Claude settings from environment or .claude/settings.json.

  Priority:
  1. Environment variables (ANTHROPIC_API_KEY, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_BASE_URL)
  2. .claude/settings.json file (for local development)
  ...
  for var in env_vars:
    ...
    if value:
      _claude_settings[var] = value
      logger.info(f'Found {var} in environment')

  # If we found API credentials in env, we're done
  if 'ANTHROPIC_API_KEY' in _claude_settings or 'ANTHROPIC_AUTH_TOKEN' in _claude_settings:
    logger.info('Using Claude credentials from environment variables')
    return _claude_settings

  # Fallback: try to load from .claude/settings.json (local dev)
  ...
  if settings_path.exists():
    try:
      with open(settings_path) as f:
        ...
        file_settings = settings.get('env', {})
        # Merge file settings (don't override env vars)
        for key, value in file_settings.items():
          if key not in _claude_settings:
            _claude_settings[key] = value
        logger.info(f'Loaded Claude settings from {settings_path}')
        return _claude_settings
    except Exception as e:
      logger.warning(f'Failed to load Claude settings: {e}')
  else:
    logger.warning(f'Claude settings file not found at {settings_path}')

  # Check if we have any credentials at all
  if 'ANTHROPIC_API_KEY' not in _claude_settings and 'ANTHROPIC_AUTH_TOKEN' not in _claude_settings:
    logger.error(
      'No Anthropic API credentials found! Set ANTHROPIC_API_KEY environment variable '
      'or create .claude/settings.json with env.ANTHROPIC_API_KEY'
    )
  ...
  def run_with_context():
    ...
    async def run_query():
      ...
      try:
        ...
        msg = 'Agent query cancelled - likely due to stream timeout or connection issue'
        result_queue.put(('error', Exception(msg)))
      except ConnectionError as e:
        logger.error(f'Connection error in agent query: {e}')
        msg = (
          f'Connection error: {e}. This may occur when tools'
          ' take longer than the stream timeout (50s).'
        )
        result_queue.put(('error', Exception(msg)))
      except BrokenPipeError as e:
        logger.error(f'Broken pipe in agent query: {e}')
        msg = f'Broken pipe: {e}. The agent subprocess communication was interrupted.'
        result_queue.put(('error', Exception(msg)))
      except Exception as e:
...
async def stream_agent_response(
  ...
  try:
    ...
    # Build subprocess env for Claude agent.
    # Claude auth is configured via ~/.claude/settings.json (created at app startup)
    # so we just need to strip OAuth creds and set up the base environment.
    claude_env = os.environ.copy()

    # Strip OAuth credentials to avoid scope issues
    claude_env.pop('DATABRICKS_CLIENT_ID', None)
    claude_env.pop('DATABRICKS_CLIENT_SECRET', None)

    # Apply base Claude settings (from env or .claude/settings.json)
    base_settings = _load_claude_settings()
    for key, value in base_settings.items():
      if value is not None:
        claude_env[key] = value

    logger.info('Agent: Claude auth configured via ~/.claude/settings.json')
          ...
          for block in msg.content:
            if isinstance(block, TextBlock):
              ...
              if block.is_error and 'Stream closed' in content:
                content = (
                  f'Tool execution interrupted: {content}.'
                  ' This may occur when operations exceed timeout'
                  ' limits or when the connection is interrupted.'
