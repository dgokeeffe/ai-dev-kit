...
# Maximum file size to include in backups (100MB)
...
async def create_backup(project_id: str) -> bool:
  ...
  # Create zip to temp file to avoid memory exhaustion on large projects
  file_count = 0
  skipped_count = 0
  ...
  with tempfile.NamedTemporaryFile(delete=False) as tmp:
    try:
          ...
          if file_path.is_file():
            ...
            try:
              ...
              if file_size > MAX_BACKUP_FILE_SIZE:
                logger.warning(f'Skipping large file in backup: {file_path} ({file_size} bytes)')
                skipped_count += 1
                continue
            except OSError as e:
            ...
            try:
              ...
              logger.warning(f'Could not add file to backup {file_path}: {e}')
              skipped_count += 1
      ...
      # Read temp file into memory for database storage
      tmp.seek(0)
      backup_data = tmp.read()
      ...
      logger.info(
        f'Backed up project {project_id} ({file_count} files, {len(backup_data)} bytes, '
      ...
      try:
        ...
        logger.warning(f'Could not delete temp file {tmp.name}: {e}')
...
def ensure_project_directory(project_id: str, template: str | None = None) -> Path:
  ...
  Args:
      ...
      template: Optional template ID for new projects
  ...
  # Write template files for new projects
  if is_new_project and template:
    write_template_files(project_dir, template)
