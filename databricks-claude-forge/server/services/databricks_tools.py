...
def load_databricks_tools():
    ...
    short_names = [n.split('__')[-1] for n in tool_names]
    logger.info(f'Loaded {len(sdk_tools)} Databricks tools: {short_names}')
                        ...
                        'text': json.dumps(
                            {
                                ...
                                'error': (
                                    ...
                                    'It may have expired (TTL: 1 hour) '
                                    'or never existed.'
                                ),
                            }
                        ),
...
def _make_wrapper(name: str, description: str, schema: dict, fn):
    ...
    async def wrapper(args: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for key, value in args.items():
                if isinstance(value, str) and value.strip().startswith(('[', '{')):
                    ...
                    try:
                        ...
                        print(
                            f'[MCP TOOL] Parsed {key} from JSON string',
            ...
            print(
                f'[MCP TOOL] Running {name} in thread pool with heartbeat...',
                file=sys.stderr, flush=True,
            )
            ...
            while True:
                try:
                    ...
                    print(
                        f'[MCP HEARTBEAT] {name} still running... '
                        f'({elapsed:.0f}s elapsed, heartbeat #{heartbeat_count})',
                        file=sys.stderr, flush=True,
                    )
                            ...
                            'content': [
                                {
                                    ...
                                    'text': json.dumps({
                                        ...
                                        'tool_name': name,
                                        'message': (
                                            f'Operation taking longer than '
                                            f'{SAFE_EXECUTION_THRESHOLD}s, '
                                            f'moved to background. Use '
                                            f'check_operation_status'
                                            f'("{op_id}") to poll.'
                                        ),
            ...
            print(
                f'[MCP TOOL] {name} completed in {elapsed:.2f}s, '
            ...
            elapsed = time.time() - start_time
            error_msg = (
                f'Tool execution cancelled after {elapsed:.2f}s '
                '(likely due to stream timeout)'
            )
            ...
            print(
                f'[MCP TOOL] {name} FAILED after {elapsed:.2f}s: {error_msg}',
                file=sys.stderr, flush=True,
            )
            ...
            err_text = (
                ...
                f'This error occurred after {elapsed:.2f}s. '
                'If this is a long-running operation, it may have '
                'exceeded the stream timeout (50s).'
            )
            return {
