...
class ActiveStream:
    ...
    Events are stored in a bounded deque (max 1000) for cursor-based retrieval.
    This prevents unbounded memory growth during long-running agent sessions.
    ...
    _pending_events: deque = field(default_factory=lambda: deque(maxlen=100))
