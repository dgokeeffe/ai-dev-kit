...
use this cluster_id by default.
"""
...
"""
  ...
  ws = workspace_url or 'WORKSPACE_URL'
...
**IMPORTANT: Before creating any Databricks resources
(tables, volumes, pipelines, jobs), propose a brief plan first.**
...
- Include: catalog/schema, table names, pipeline names, pipeline ids,
  volume paths, all databricks resources created name and ID
Use it as storage to track all the resources created in the project,
and be able to update them between conversations.

## Tool Usage

- **Always use MCP tools** - never use CLI commands, curl, or SDK code when an MCP tool exists
- MCP tool names: `mcp__databricks__<tool_name>` (e.g., `mcp__databricks__execute_sql`)
...
**CRITICAL: After creating ANY Databricks resource, ALWAYS provide a
clickable link so the user can verify it.**

Use these URL patterns (workspace URL: `{ws}`):

| Resource | URL Pattern |
|----------|-------------|
| Table | `{ws}/explore/data/{{catalog}}/{{schema}}/{{table}}` |
| Volume | `{ws}/explore/data/volumes/{{catalog}}/{{schema}}/{{volume}}` |
| Pipeline | `{ws}/pipelines/{{pipeline_id}}` |
| Job | `{ws}/jobs/{{job_id}}` |
| Notebook | `{ws}#workspace{{path}}` |

**Example response after creating resources:**

> Data generation complete! I created:
> - **Volume:** [raw_data]({ws}/explore/data/volumes/ai_dev_kit/demo_schema/raw_data)
...
| **Volume** | `GRANT READ VOLUME, WRITE VOLUME ON VOLUME c.s.vol TO \`account users\`` |
...
1. **IMMEDIATELY load the relevant skill** - NON-NEGOTIABLE. Load skill FIRST
